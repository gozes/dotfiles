return {
  ...
  config = function()
    ...
    commands.add('ZkUnlinkedMentionFrom', function(options)
      options = vim.tbl_extend('force', { mentionedBy = { vim.api.nvim_buf_get_name(0) }, linkedBy = { vim.api.nvim_buf_get_name(0) } }, options or {})
      zk.edit(options, { title = 'Zk Unlinked Mention From' })
    end)
    commands.add('ZkUnlinkedMentionTo', function(options)
      options = vim.tbl_extend('force', { mention = { vim.api.nvim_buf_get_name(0) }, linkTo = { vim.api.nvim_buf_get_name(0) } }, options or {})
